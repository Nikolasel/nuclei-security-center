...
Compose runs unmodified [Garage](https://garagehq.deuxfleurs.fr/) `v2.3.0` (`dxflrs/garage`, AGPL-3.0)
as the local S3 endpoint. That is appropriate for development and for self-hosting the upstream image
as-is. Revisit the choice if NSC ever patches Garage and ships that modified build: AGPL-3.0 would
then require offering the corresponding Garage source. The application client stays `minio-go`; it
