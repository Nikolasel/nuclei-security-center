...
| `GET /v1/capabilities` | `{ nuclei_version, templates_commit, naabu_scan_type }` — polled by the backend for node liveness (#98) and node-effective discovery defaults (#271); `templates_commit` is the digest of the active bundle (empty until one is pushed), used to detect drift before dispatch; `naabu_scan_type` is the node's normalized `NAABU_SCAN_TYPE` (`syn` default, `connect` unprivileged fallback) |
...
- **Node registry:** scanner nodes live in a DB-backed registry (`scanner_nodes`) managed by
  ...
  **seed** it on first boot. Each node serves a set of CIDRs (a node with none is a catch-all);
  CIDRs are non-overlapping, so a target's IP maps to exactly one node — no load-balancing. Each
...
- **BFF token custody:** OIDC access/refresh tokens live server-side in the backend; the
  ...
  SPA document requests whose cookie host (hostname plus non-default port) does not match it are
  redirected there before an auth-state cookie is set, so `localhost` vs `127.0.0.1` cannot split
  the OIDC dance across two cookie hosts (#298). Scheme is ignored so a TLS-terminating ingress
  cannot 302-loop. `/healthz` and other `/api/*` routes are not redirected.
- **Browser mutation protection:** cookie-authenticated state-changing requests must carry an
  ...
  do not depend on ambient browser cookies and are exempt from the browser-origin header.
   ...
   `APP_BASE_URL` is missing or malformed and also when server-side session revocation fails —
   a transient Postgres failure yields `503` with `Retry-After: 1`, preserves the browser cookie
   so logout can be retried, and does not present success or an IdP redirect until the row is
   gone, so a copied cookie cannot be replayed after a user saw logout succeed (#268).
