...
| `NAABU_SCAN_TYPE` | scanner | `syn` | Node **default** naabu port-scan mode when a scan policy leaves `discovery_scan_type` unset: `syn` (needs `CAP_NET_RAW` + libpcap, bundled in the image) or `connect` (unprivileged TCP connect scan, useful when `NET_RAW` is unavailable). Host discovery defaults on for `syn` and off for `connect`; a policy can override that independently with `discovery_host_discovery=true|false`. An explicit `syn` port scan or host-discovery pass still fails closed on a node without raw sockets. |
