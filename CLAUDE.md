...
internal/backend   Orchestrator (dispatch/poll/ingest + raw archive) + Scheduler (cron ticker) + ScannerClient + scanner node registry (nodes.go config-seeder + nodes_http.go admin API; DB-backed, dispatch picks the node whose CIDRs match the target — #22) + ObjectStore (objectstore.go, S3/MinIO) + HTTP API + OIDC/BFF auth (auth.go, authz.go) + audit-log middleware (audit.go)
